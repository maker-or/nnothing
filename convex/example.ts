    ...
    for (const stage of stages) {
      ...
      const stageTrace = langfuse.trace({
        ...
        metadata: {
          ...
          parent_trace_id: agentTrace.id,
          ...baseProps,
        },
      });

      // Create generation for this stage
      const stageGeneration = langfuse.generation({
        name: `stage-generation-${stage.title}`,
        model: 'gemini-2.5-flash',
        input: stagePrompt,
        traceId: stageTrace.id,
        metadata: {
          stage_title: stage.title,
          ...baseProps,
        },
...
// Allow multiple steps for tool usage and final generation
            ...
            switch (chunk.type) {
              ...
              case 'tool-call':
                ...
                if (chunkAny.toolName) {
                  ...
                  // Log tool call event to Langfuse
                  langfuse.event({
              ...
              case 'tool-result':
                ...
                if (chunkAny.toolName) {
                  ...
                  // Log tool result event to Langfuse
        ...
        if (finishReason === 'tool-calls' && (!finalText || finalText.trim().length === 0)) {
          ...
          if (toolResults && toolResults.length > 0) {
            ...
            // Create generation for synthesis
            const synthesisGeneration = langfuse.generation({
              name: `synthesis-generation-${stage.title}`,
              model: 'gemini-2.5-flash',
              input: toolSummaryForSynthesis,
              traceId: stageTrace.id,
              metadata: {
                stage_title: stage.title,
                phase: 'synthesis',
...
Generate a complete, detailed response now.`,
            ...
            // End synthesis generation
            synthesisGeneration.end({
              output: {
                synthesized_text: synthesizedText,
                text_length: synthesizedText?.length || 0,
              },
            });
          }
        }
...
Your output must strictly follow the provided schema structure.`
    ...
    // Complete the main agent trace
    agentTrace.update({
      output: {
        created_stages: stageIds.length,
        stage_ids: stageIds,
        total_stages_requested: course.course?.stages?.length || 0,
      },
    });
